...
from src.model_processing.create_model import TrainingModel
...
def split_folds(subject_list, testing_subject):
    print("outer_original_split_folds")
    """
...
def get_filename_list(path):
    print("outer_original_get_filename_list")
    """
...
def get_configFile_list(path):
    print("outer_original_get_configfile_list")
    file_count = 0
    configFile_list = []
...
def get_label_subject(path,label_position ,class_names, subject_list):
    ...
    indexes=class_names.index(labels[0])
    ...
    return labels[0], indexes, subjects[0], label_position
...
def training(data, testing_subject, config_name):
    ...
    for file_name in filename_list_all:
        label, indexes, subject,label_position =get_label_subject(file_name,label_position, class_names, subject_list)
        label_list_all.append(label)
        label_index_list_all.append(indexes)
    ...
    for indexes, file in enumerate(filename_list_all):
        if subject_list_all[indexes] == folds[0]['test'][0]:
            test_file_name_list.append(file)
            test_label_list.append(label_list_all[indexes])
            test_index_list.append(label_index_list_all[indexes])
        else:
            train_file_name_list.append(file)
            train_label_list.append(label_list_all[indexes])
            train_index_list.append(label_index_list_all[indexes])
    print("------------------------------------")
    ...
    with open('%s/%s_test_%s/%s_test_%s_classes-names.csv' % (results_path, model_type, test_subject, model_type, test_subject), 'w') as f:
        writer = csv.writer(f)
        for indexes, item in enumerate(class_names):
