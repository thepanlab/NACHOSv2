...
def save_gradcam_output(img_path, heatmap, cam_path, alpha=0.4):
    """
    Saves the Grad-CAM output to a file.
    ...
    # Rescales heatmap to a range 0-255
    heatmap = np.uint8(255 * heatmap)

    # Uses jet colormap to colorize heatmap
    jet = cm.get_cmap("jet")

    # Uses RGB values of the colormap
    ...
    # Superimposes the heatmap on original image
...
def main(config = None):
    ...
    # If a folder of images, reads the folder or else just use the one image
    img_addrs = get_images(config["input_img_address"])

    # Processes every image
    for img_addr_i in img_addrs:
        # Preprocesses the image
        img_processed = preprocessing(img_addr_i)

        # Generates a heatmap from the model
