...
def get_images(data_input, class_names):
    ...
    for subject in data_input:     
        ...
        # Gets all of the subject's image subdirectories
        image_paths[subject] = []
...
def read_models(model_input):
    ...
    # Checks that the input models exist
...
def output_results(config, prediction_results, timing_results, input_filepaths, class_names, out_vals):
    ...
    for model in prediction_results:
        ...
        for subject in prediction_results[model]:
            ...
            # Prints the prediction probability values
            ...
            # Prints the prediction index values
            preds = preds.indexesmax(axis=1)
            ...
            if config['use_true_labels']:
                ...
                # Prints the true label index values
...
def main(config = None):
    ...
    # Sets eager execution
    tf_config = tf.compat.v1.ConfigProto()
    tf_config.gpu_options.allow_growth = True
    tf.config.run_functions_eagerly(True)

    # Checks that output directories exist
